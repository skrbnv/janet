...
class ResNet(nn.Module):
    def __init__(self,
        ...
        self.convA = nn.Conv2d(1,
                               32,
                               kernel_size=3,
                               stride=(1, 3),
                               padding=0,
                               bias=False)
        self.convB = nn.Conv2d(1,
                               32,
                               kernel_size=3,
                               stride=(1, 3),
                               padding=0,
                               bias=False)
        self.convC = nn.Conv2d(1,
                               32,
                               kernel_size=3,
                               stride=(1, 3),
                               padding=0,
    ...
    def _forward_impl_custom(self, x):
        # (64x1x64x192)
        x1 = F.pad(x, (1, 1, 1, 1))
        x2 = F.pad(x, (0, 2, 1, 1))
        x3 = F.pad(x, (2, 0, 1, 1))
        a, b, c = self.convA(x1), self.convB(x2), self.convC(x3)
        a, b, c = self.bnA(a), self.bnB(b), self.bnC(c)
        a, b, c = self.relu(a), self.relu(b), self.relu(c)
        # (64x3x32x64x64)
