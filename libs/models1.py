...
class Conv2WMul_v2g(nn.Module):
    def __init__(self,
        ...
        #self.wm = WMul_v2g(planes_out, input_shape[0], input_shape[1],
        #                   residual)
...
class Extractor_v2g(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.adjust = Conv2WMul_v2g(
            input_shape=(64, 192),
            planes_in=1,
            planes_out=128,
            kernel_size=7,
        ...
        seq = [
            Conv2WMul_v2g(input_shape=(int(64 / 4**(i + 1)),
                                       int(64 / 4**(i + 1))),
                          planes_in=128 * 2**i,
...
class Classifier_v2g(nn.Module):
...
class Janet_v2g(nn.Module):
    def __init__(self, num_classes) -> None:
        super().__init__()
        self.extractor = Extractor_v2g()
        self.classifier = Classifier_v2g(size_in=1024, size_out=num_classes)
