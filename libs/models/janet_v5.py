...
class Extractor(nn.Module):
    def __init__(self, num_heads=8) -> None:
        ...
        self.conv2 = nn.Conv2d(1,
                               ...
                               padding=2,
                               bias=False)
        self.bn2 = nn.BatchNorm2d(num_heads)
        self.stairs = nn.Sequential(*[
            self.create_conv_block(num_heads * 4**i, num_heads * 4**(i + 1))
            for i in range(4)
        ])
        self.avgpool2 = nn.AdaptiveAvgPool2d(1)

    def create_conv_block(self,
                          planes_in,
                          planes_out,
                          kernel_size=5,
                          stride=2,
...
class Janet(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        num_heads = 4
        self.extractor = Extractor(num_heads)
        self.classifier = Classifier(size_in=num_heads * 2**8,
                                     size_out=630,
                                     midsize=num_heads * 2**7,
                                     dropout=.5)
