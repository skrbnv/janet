...
speakers = [ name for name in os.listdir(ORIGIN_FOLDER) if os.path.isdir(os.path.join(ORIGIN_FOLDER, name)) ]
...
for speaker in speakers:
    ...
    for currentPath, subdirs, filenames in os.walk(speakerFolder):
        ...
        for filename in filenames:
            ...
            if candidateFile.endswith(extensions) and os.stat(candidateFile).st_size > 0:
                ...
                # Generating list
                copyCandidates.append([candidateFile, targetTrainingFile, targetTestingFile])
    ...
    for copyCandidate in copyCandidates:
        if traningExamplesCopied < trainingLimitPerSpeaker:
            if dryrun:
                print("Copy traning example:",copyCandidate[0], ">>>",copyCandidate[1])
            else: # not dryrun
                try: 
            ...
            if testingExamplesCopied < testingLimitPerSpeaker:
                if dryrun:
                    print("Copy testing example:",copyCandidate[0], ">>>",copyCandidate[2])
                else: #not dryrun
                    try: 
