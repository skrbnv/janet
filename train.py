...
from libs.models.resnet import Model
...
DATASET_TRAIN = "glued-2s-mel80-plain-train.dt"
DATASET_VALIDATE = "glued-2s-mel80-plain-validate.dt"
DATASET_TEST = "TIMIT-test.dt"
SPECTROGRAMS_CACHE = "/media/my3bikaht/EXT4/datasets/TIMIT2/cache/glued-2s-mel80-224x224_plain"
# VISUALISATION
VIZ_DIR = "./visualization/"
# CHECKPOINTS
RUN_ID = 'o3cxx0nc'
CHECKPOINTFILENAME = "./checkpoints/224x224_resnet50_pretrained-benchmark-negativefixed-2bcont.dict"
# SUBSETS
USE_SUBSETS = True
SUBSET_SPEAKERS = 64
...
if WANDB:
    if RESUME:
        # swept-lake-205: https://wandb.ai/my3bikaht/triplets-timit-simple/runs/o3cxx0nc
...
model = Model()
#for name, param in model.basemodel.named_parameters():
...
#torchinfo.summary(model, ((64, 1, 128, 256), (64, 1, 128, 256)))
...
for epoch in range(initial_epoch, EPOCHS_TOTAL):
    ...
    for batch_speakers in D.speakers_iterator(SUBSET_SPEAKERS, True):
        ...
        positives = []
        negatives = []
        for j, speaker in enumerate(Dsub.get_unique_speakers()):
            a, p, n = _tpl.generate_triplets(Dsub, TRIPLETSPERCLASS, speaker)
            anchors.extend(a)
            positives.extend(p)
            negatives.extend(n)
