...
class DepthWiseConv2d(nn.Module):
	def __init__(self, in_channels, out_channels, kernel_size, stride, padding, bias = False):
		super().__init__()
		self.conv = nn.Conv2d(1, out_channels=out_channels, kernel_size=kernel_size, stride=stride, padding=padding, bias=bias)
		self.bn = nn.BatchNorm2d(out_channels)
		self.relu = nn.ReLU()
	def forward(self,x):
		# split input (Nc, W, H) into Nc inputs (W, H)
		# run Conv2d() over each input
		out = self.conv(x[0])
		for i in range(1, x.shape[0]):
			out = vstack(out, self.conv1(x[i]))
		out = self.bn(out)
		out = self.relu(out)
		return out		
...
class Wideneck(nn.Module):
	def __init__(self, inplanes=128, width=64, bias = False):
		...
		self.conv1x1A = nn.Conv2d(inplanes, width, kernel_size=1, stride=1, padding=0, bias=bias)
		self.bnA = nn.BatchNorm2d(width)
		self.dwconv = DepthWiseConv2d(width, width, kernel_size=3, stride=1, padding=1, bias=False)
		self.conv1x1Z = nn.Conv2d(width, inplanes, kernel_size=1, stride=1, padding=0, bias=bias)
		self.bnZ = nn.BatchNorm2d(inplanes)
		self.relu = nn.ReLU()
