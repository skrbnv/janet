...
DATASET_TARGET = "/media/my3bikaht/EXT4/datasets/TIMIT2/datasets/glued-1_92s-mel64-plain-train.dt"
...
# either 'glue' non-silent parts into one then slice or
...
def multiprocess_sample(sample):
    ...
    slices_raw = _fn.generate_slices(audioEmpzd,
                                     length=length,
                                     step=step,
                                     trim=trim,
    ...
    inner_counter = 0
    records = []
    for (eachslice, augm), eachindex in zip(slices, indices):
        if inner_counter == MAX_SPECTROGRAMS_PER_SAMPLE:
            break
        spectrogram = _fn.generate_spectrogram(eachslice,
                                               sr=16000,
                                               n_mels=MEL_BANKS)
        # transform to [-1;1]
        # our spectrograms' values lay within range [-80;0]
        # first we add 40 => [-40;40]
        # then we divide by 40 => [-1;1]
        spectrogram += 40
        spectrogram /= 40
        if FORCE_SHAPE:
